...
class RequestListCreateView(APIView):
    ...
    def get(self, request):
        ...
        if not is_member_approved(request.user, community_id):
            return Response({'detail': 'No perteneces a la comunidad.'}, status=status.HTTP_403_FORBIDDEN)

        queryset = (
            Request.objects.filter(community_id=community_id)
    ...
    def post(self, request):
        ...
        if not is_member_approved(request.user, community.id):
            return Response({'detail': 'No perteneces a la comunidad.'}, status=status.HTTP_403_FORBIDDEN)
...
class RequestDetailView(APIView):
    ...
    def get(self, request, request_id):
        ...
        if not is_member_approved(request.user, obj.community_id):
            return Response({'detail': 'No perteneces a la comunidad.'}, status=status.HTTP_403_FORBIDDEN)
        ...
        can_accept = obj.status == Request.Status.OPEN and obj.created_by_user_id == request.user.id
        can_close = obj.created_by_user_id == request.user.id and obj.status in [
            Request.Status.OPEN,
    ...
    def patch(self, request, request_id):
        ...
        if obj.created_by_user_id != request.user.id:
...
class RequestCloseView(APIView):
    ...
    @extend_schema(
        ...
        description='Cierra una petición como resolved o cancelled (solo creador).',
    )
...
class OfferListCreateView(APIView):
    ...
    @extend_schema(
        ...
        description='Devuelve ofertas de una petición. Solo el creador puede ver todas.',
        responses={200: VolunteerOfferSerializer(many=True)},
    )
    def get(self, request, request_id):
        req = get_object_or_404(Request, id=request_id)
        if req.created_by_user_id != request.user.id:
            return Response({'detail': 'Solo el creador puede ver las ofertas.'}, status=status.HTTP_403_FORBIDDEN)
        offers = (
